...
import { getUniqueTableName, getUniqueColumnName, validateColumnName, validateTableName } from '../schemas/sandbox.schema';
import { 
  createNewColumnId, 
  insertColumnAt, 
  deleteColumnFrom, 
  ...
  persist(
    (set, get) => ({
      ...
      insertRowBelow: (targetRowId) => {
        ...
        set(state => ({
          tables: state.tables.map(t => t.id === state.activeTableId ? insertRowAt(t, targetRowId, newRow) : t)
        }));
        return null;
      },
      ...
      appendColumn: () => {
        ...
        set(state => ({
          tables: state.tables.map(t => t.id === state.activeTableId ? appendColumn(t, newCol) : t)
        }));
        return null;
      },
      ...
      deleteColumn: (colId) => {
        ...
        set(state => ({
          tables: state.tables.map(t => t.id === state.activeTableId ? deleteColumnFrom(t, colId) : t)
        }));
        return null;
      },
      ...
      insertColumnBeside: (targetColId) => {
        ...
        set(state => ({
          tables: state.tables.map(t => t.id === state.activeTableId ? insertColumnAt(t, targetColId, newCol) : t)
        }));
        return null;
      },
      ...
      replaceActiveTable: (updatedTable) => {
        set(state => ({
          tables: state.tables.map(t => t.id === state.activeTableId ? { ...t, ...updatedTable } : t)
        }));
      },
