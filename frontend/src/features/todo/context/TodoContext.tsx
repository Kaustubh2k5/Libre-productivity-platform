...
import type { Task, TimetableItem, GoalTemplate, TodoContextType, Priority, TaskStatus } from '../types';
...
export const TodoProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [tasks, setTasks] = useState<Task[]>(() => {
    ...
    return [
      { id: 't-1', title: 'Complete stamina diagnostics', priority: 'high', tags: ['Onboarding'], status: 'concluded', createdAt: new Date(Date.now() - 43200000).toISOString(), hours: 1.5, timeSlot: '08:00 - 09:30' },
      { id: 't-2', title: 'Configure Sandbox columns', priority: 'high', tags: ['Sandbox'], status: 'active', createdAt: new Date().toISOString(), hours: 1, timeSlot: '11:00 - 12:00' },
      { id: 't-3', title: 'Draft telemetry design', priority: 'medium', tags: ['Design'], status: 'active', createdAt: new Date().toISOString(), hours: 2, timeSlot: '13:00 - 15:00' },
      { id: 't-4', title: 'Review digital checkout & triggers', priority: 'low', tags: ['Integration'], status: 'backlog', createdAt: new Date().toISOString(), hours: 1, timeSlot: '15:30 - 16:30' },
      { id: 't-5', title: 'Practice breath control session 2', priority: 'low', tags: ['Mindfulness'], status: 'backlog', createdAt: new Date().toISOString(), hours: 0.5, timeSlot: '17:00 - 17:30' }
    ];
  });
  ...
  const [timetable, setTimetable] = useState<TimetableItem[]>(() => {
    ...
    return [
      { id: 'tt-1', timeSlot: '08:00 - 09:30', activity: 'Strategic Objectives & Focus Inbox', category: 'Deep Work' },
      { id: 'tt-2', timeSlot: '10:00 - 11:30', activity: 'Backend Core Integration Refinement', category: 'Engineering' },
      { id: 'tt-3', timeSlot: '12:00 - 13:00', activity: 'Technical Synchronicities & Metrics', category: 'Collaboration' },
      { id: 'tt-4', timeSlot: '14:00 - 15:30', activity: 'Polishing High Contrast Kanban Boards', category: 'Development' },
      { id: 'tt-5', timeSlot: '16:00 - 17:00', activity: 'Performance Diagnostics & Outlining Stale Tasks', category: 'Review' }
    ];
  });
  ...
  const syncTemplatesAndCache = async () => {
    ...
    try {
      ...
      console.warn("⚠️ GET /api/goal-templates failed or is unreachable. Checking cache/sandbox configuration:", err.message);
  ...
  useEffect(() => {
    if (loggingTask) {
      ...
      if (template) {
        ...
        const token = getAccessToken() || 'jwt_secure_access_token_token_abc_123';
        console.log(`📤 GET /api/goal-templates - Checking server for tag "${primaryTag}" with access token:`, token);
        
        axios.get('/api/goal-templates', {
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
          .then(res => {
            if (res.data && Array.isArray(res.data)) {
              const matched = res.data.find((t: any) => t.name.toLowerCase() === primaryTag.toLowerCase());
              if (matched) {
                const mapped = {
                  id: matched.id,
                  name: matched.name,
                ...
                try {
                  ...
                  localStorage.setItem('libre_task_templates', JSON.stringify([...currentCache, mapped]));
                } catch (e) {}
                return;
              }
            }
          ...
          .catch(err => {
            console.warn(`⚠️ GET /api/goal-templates failed during verification of tag "${primaryTag}":`, err.message);
            // Treat as custom tag
            setFetchedTemplate(null);
            setLogFields({
              notes: '',
  ...
  const addTask = (params: {
    ...
    const { title, priority, tagType, selectedGoalTag, customTagInput, startTime, endTime } = params;
    ...
    setTasks(prev => prev.map(t => {
      if (t.id === taskId) {
        if (t.status !== targetStatus) {
          triggerToast(`Moved task to ${targetStatus === 'backlog' ? 'Backlog' : 'Active Focus'}`);
        }
  ...
  const addTimetableSlot = (params: {
    ...
    const token = getAccessToken() || 'jwt_secure_access_token_token_abc_123';
    console.log("📤 POST /api/timetable-slots - Sending payload with accessToken:", token, slotPayload);
    
    axios.post('/api/timetable-slots', slotPayload, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
      ...
      .catch(err => {
        console.warn("⚠️ POST /api/timetable-slots dispatch failed (offline mode fallback):", err.message);
      });
  ...
  const handleCompleteLogging = () => {
    ...
    const primaryTag = loggingTask.tags[0] || 'General';
    const targetTable = sandboxTables.find(t => t.name.toLowerCase() === primaryTag.toLowerCase())
    ...
    if (fetchedTemplate) {
      ...
      if (targetTable) {
        ...
        loggedRowData['Finished Date'] = logFields.dateFinished || new Date().toISOString().split('T')[0];
      }
    }
    ...
    const token = getAccessToken() || 'jwt_secure_access_token_token_abc_123';
    console.log("📤 POST /api/goal-logs - Dispatched payload to server with token:", token, loggingPayload);

    axios.post('/api/goal-logs', loggingPayload, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
      ...
      .catch(err => {
        console.warn("⚠️ POST /api/goal-logs dispatch failed (offline mode fallback or expected sandbox iframe 404):", err.message);
      });
    ...
    setLoggingTask(null);
    triggerToast(`Logged to "${fetchedTemplate?.name || targetTable?.name || primaryTag}" & Concluded`);
  };
