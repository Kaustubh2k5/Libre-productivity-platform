...
export const signupStartService = async ({
  email,
  password,
  clientId,
}: SignupInput) => {
...
export const signupVerifyService = async ({
  email,
  otp,
  clientId,
}: SignupVerifyInput) => {
   ...
   * get OTP state
   */
  const otpData = await OtpRepository.getOtp(email);
   ...
   */
  const otpMatches = hashToken(otp) === otpData.otpHash;

  if (!otpMatches) {
    await OtpRepository.incrementOtpAttempts(email);
  ...
  await OtpRepository.clearOtpAttempts(email);
